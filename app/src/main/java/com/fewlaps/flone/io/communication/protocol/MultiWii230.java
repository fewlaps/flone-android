package com.fewlaps.flone.io.communication.protocol;
...
public class MultiWii230 extends MultirotorData {
    ...
    private DroneSensorData sensorInformation = new DroneSensorData();
    ...
    public void Request(int code) {
        this.sendRequestMSP(requestMSP(code));
    }

    public void Request(int[] codes) {
        for (int code : codes)
            Request(code);
    }
    ...
    public void evaluateCommand(byte cmd, int dataSize) {
        ...
        switch (icmd) {
            ...
            /*
             * if ((multiCapability & 1) > 0) multi_Capability.RXBind = true; if
			 * ((multiCapability & 4) > 0) multi_Capability.Motors = true; if
			 * ((multiCapability & 8) > 0) multi_Capability.Flaps = true;
			 * 
			 * if ((multiCapability & 0x80000000) > 0) multi_Capability.ByMis =
			 * true;
			 * 
			 * break;
			 */
...
//                Log.d("aaa", "MSP_ATTITUDE: angx = " + angx + ",angy = " + angy + ",head = " + head);
		...
		/*
         * case MSP_SERVO_CONF: // min:2 / max:2 / middle:2 / rate:1 for (i = 0;
		 * i < 8; i++) { ServoConf[i].Min = read16(); ServoConf[i].Max =
		 * read16(); ServoConf[i].MidPoint = read16(); ServoConf[i].Rate =
		 * read8(); } break;
		 */
            case MSP_MISC:
...
//            Log.v("READ", "END OF THE MESSAGE ---------------");
    ...
    @Override
    public void SendRequestMSP_SET_PID(float confRC_RATE, float confRC_EXPO,
    ...
    int[] requests = new int[]{0, MSP_ATTITUDE, MSP_ALTITUDE, MSP_RAW_GPS,
            MSP_BOX, MSP_RAW_IMU};
    final int[] requestsOnce = new int[]{MSP_IDENT, MSP_BOXNAMES, MSP_PID,
            MSP_BOX, MSP_MISC};
    int[] requestsPeriodical = new int[]{MSP_STATUS, MSP_COMP_GPS,
            MSP_ANALOG, MSP_SERVO, MSP_MOTOR, MSP_RC, MSP_DEBUG};
    ...
    public void SendRequestMSP_WP(int Number) {
        ...
        sendRequestMSP(requestMSP(MSP_WP,
                payload.toArray(new Character[payload.size()])));
        Log.d("aaa",
                "MSP_WP (SendRequestGetWayPoint) " + String.valueOf(Number));
    }
    ...
    public void SendRequestMSP_SET_SERIAL_BAUDRATE(int baudRate) {
        ...
        sendRequestMSP(requestMSP(MSP_SET_SERIAL_BAUDRATE,
                payload.toArray(new Character[payload.size()])));
	 ...
	 * 16) & 0xFF)); payload.add((char) ((w.Lat >> 24) & 0xFF));
	 * 
	 * payload.add((char) (w.Lon & 0xFF)); payload.add((char) ((w.Lon >> 8) &
	 * 0xFF)); payload.add((char) ((w.Lon >> 16) & 0xFF)); payload.add((char)
	 * ((w.Lon >> 24) & 0xFF));
	 * 
	 * payload.add((char) (w.Altitude & 0xFF)); payload.add((char) ((w.Altitude
	 * >> 8) & 0xFF)); payload.add((char) ((w.Altitude >> 16) & 0xFF));
	 * payload.add((char) ((w.Altitude >> 24) & 0xFF));
	 * 
	 * payload.add((char) (w.Heading & 0xFF)); payload.add((char) ((w.Heading >>
	 * 8) & 0xFF));
	 * 
	 * payload.add((char) (w.TimeToStay & 0xFF)); payload.add((char)
	 * ((w.TimeToStay >> 8) & 0xFF));
	 * 
	 * payload.add((char) w.NavFlag);
	 * 
	 * sendRequestMSP(requestMSP(MSP_SET_WP, payload.toArray(new
	 * Character[payload.size()])));
	 * 
	 * Log.d("aaa", "MSP_SET_WP " + String.valueOf(w.Number) + "  " +
	 * String.valueOf(w.Lat) + "x" + String.valueOf(w.Lon) + " " +
	 * String.valueOf(w.Altitude) + " " + String.valueOf(w.NavFlag)); }
	 */
    ...
    public void SendRequestMSP_SET_HEAD(int heading) {
        ...
        sendRequestMSP(requestMSP(MSP_SET_HEAD,
                payload.toArray(new Character[payload.size()])));
    ...
    public void SendRequestMSP_SET_MOTOR(byte motorTogglesByte) {
        ...
        payload = new ArrayList<Character>();
        motorTogglesByte = (byte) (motEnable[0] + motEnable[1] * 2
                + motEnable[2] * 4 + motEnable[3] * 8 + motEnable[4] * 16
                + motEnable[5] * 32 + motEnable[6] * 64 + motEnable[7] * 128);
        payload.add((char) (motorTogglesByte));
        // toggleMotor=false;
        sendRequestMSP(requestMSP(MSP_SET_MOTOR,
                payload.toArray(new Character[payload.size()])));
    ...
    public void SendRequestMSP_SET_SERVO_CONF() {
		 ...
		 * 8) & 0xFF));
		 * 
		 * payload.add((char) (ServoConf[i].Max & 0xFF)); payload.add((char)
		 * ((ServoConf[i].Max >> 8) & 0xFF));
		 * 
		 * payload.add((char) (ServoConf[i].MidPoint & 0xFF));
		 * payload.add((char) ((ServoConf[i].MidPoint >> 8) & 0xFF));
		 * 
		 * payload.add((char) ServoConf[i].Rate); }
		 * sendRequestMSP(requestMSP(MSP_SET_SERVO_CONF, payload.toArray(new
		 * Character[payload.size()])));
		 */
